...
os.environ['CUDA_VISIBLE_DEVICES'] = '2'  # CUDA configuration
import torch
import torch.nn as nn
from models.cifar10.vgg16 import CNN_ReluX, SpikingVGG16OL
...
if __name__ == '__main__':
    ...
    train_record_path = 'spiking_vgg16_t{0}_Tw{1}_tau10_L2_p{2}_online_relu2'.format(Tencode, Twarm, p)
    coeff_local = [args.local_coefficient] * 15 # Local loss coefficient
    test_acc_history = []
    train_acc_history = []
    ...
    # Init ANN and SNN
    ann = CNN_ReluX()
    snn = SpikingVGG16OL(Tencode)
    ann = ann.to(device)
    snn = snn.to(device)

    # load pre-trained ANN model
    checkpoint = torch.load(ann_ckp_dir + 'checkpoint/vgg16_relu2_baseline_sgd.pt')
    ...
    for epoch in range(num_epochs):
        ...
        if acc_test > best_test_acc:
            ...
            torch.save(state, snn_ckp_dir + 'checkpoint/spiking_vgg16_t{0}_Tw{1}_tau10_L2_p{2}_online_relu2.pt'.format(Tencode, Twarm, p))
